...
##########
#PISTO MOBIL
directori = "C:\\Users\\surfi\\Desktop\\3r FÍSICA\\2-Termo\\Simulació\\Q2\\Imatges"
nom_archiu = "dades_R0.05_T500.txt"
ruta_archiu = os.path.join(directori, nom_archiu)

def guardar(aa, bb):
    with open(ruta_archiu, "a") as archiu:
        archiu.write(f"{aa}\t{bb}\n")
...
piston_pos_prev = L/2 
piston_pos_current = L/2 #POSICIO DEL PISTO A L'INICI L/2
piston_pos_next=0

pos_pisto=[piston_pos_prev,piston_pos_current,piston_pos_next]
#xPisto=[] #VECTOR PER GUARDAR LA POSICIÓ DEL PISTO
#temps=[] #VECTOR PER GUARDAR LA ITERACIÓ
sumvel=0

def verlet(y,g,dt,sumvel,M):
    y[2]=2*y[1]-y[0]-g*dt**2+2*sumvel*dt/M
    y[0]=y[1]
    y[1]=y[2]
    return y[2]
...
while True:
    ...
    for i in range(Natoms):
        loc = apos[i]
        sumvel=0
        if loc.y > piston_pos_current:
            apos[i].y=piston_pos_current
            p[i].y =-abs(p[i].y) #ACTUALITZACIÓ DE LA P DE LA PARTICULA QUE HA IMPACTAT
            sumvel+=abs(p[i].y) #SI UNA PARTICULA ARRIBA AL PISTO CALCULEM L'INCREMENT DE MOMENT 
        ...
        piston_pos_current=verlet(pos_pisto,g,dt,sumvel,M)
            
        
    
    
    if kk%10==0:        
      guardar(kk,piston_pos_current)  
        
      #xPisto.append(piston_pos_current)
      #temps.append(nhisto)
    ...
    """
    plt.plot(temps,xPisto,"." ,label="posició del pistó")
    plt.savefig(f"C:\\Users\\surfi\\Desktop\\3r FÍSICA\\2-Termo\\Simulació\\Q2\Imatges\\imatge{k}.png")
    plt.close()
    """
    ...
    """if kk%500==0:
        ...
        temps.append(nhisto)
        plt.plot(temps,xPisto,"." ,label="posició del pistó")
        plt.savefig(f"C:\\Users\\surfi\\Desktop\\3r FÍSICA\\2-Termo\\Simulació\\Q2\Imatges\\imatge{kk}.png")
        plt.close()  
    """
